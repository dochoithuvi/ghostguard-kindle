...
# Installs/checks KOReader, optionally installs SimpleUI, refreshes the current
# multi-package KPM repository, installs GhostGuard 0.6.14, syncs the latest
# SimpleUI Tools bridge, then launches GhostGuard.
...
install_simpleui(){
  ...
  say 3 "[2/7] Cai SimpleUI..."; rm -rf "$D" "$Z"; mkdir -p "$D"
  get "$U" "$Z" || return 1
  command -v unzip >/dev/null 2>&1 || return 1
  unzip -q "$Z" -d "$D" >/dev/null 2>&1 || return 1
  S=""
...
log "Expected: GhostGuard $GG_EXPECT + latest SimpleUI Tools bridge"
log "Date: $(date)"
log "KPM=$KPM"
log "========================================"

say 1 "DCPRO GhostGuard Installer v12.1"
say 2 "GitHub KPM v2 + SimpleUI Tools fix"
