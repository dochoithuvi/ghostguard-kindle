...
# v0.8.1 Library icon fix:
# SH_Integration accepts either data:image or a real file path as `thumbnail`.
# Use the already-packaged cover file directly, and make it writable because
# SH_Integration's current icon validator requires R_OK|W_OK for path icons.
...
# Stage outside /documents so the scanner never sees a half-written launcher.
LAUNCHER_TMP="$DATA/DCPRO_GhostGuard.launcher.$$"
rm -f "$LAUNCHER_TMP" 2>/dev/null || true
# Replace the large embedded-base64 Icon line with the stable on-device path.
...
# Force SH_Integration to observe a true DELETE -> ADD transition. An UPDATE of
# an already-blank thumbnail may leave the Kindle Library cache visually stale.
...
echo "GhostGuard v0.8.1 installed. KOReader root: $KO_ROOT"
...
echo "Restart KOReader once after upgrading to v0.8.1."
exit 0
