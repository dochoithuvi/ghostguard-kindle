...
p = sys.argv[1]
s = open(p, encoding="utf-8").read()
for old in ('0.6.13', '0.6.14', '0.6.17'):
    s = s.replace('version = "' + old + '"', 'version = "0.6.17"', 1)
open(p, "w", encoding="utf-8").write(s)
PY
...
grep -q 'version = "0.6.17"' "$TMP/payload/dcghostguardpro.koplugin/defaults.lua"
